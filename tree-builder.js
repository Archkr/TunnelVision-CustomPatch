...
async function withConnectionProfile(fn) {
    getSettings();
    const targetProfileId = getConnectionProfileId();
    if (!targetProfileId) return fn();

    const targetProfileName = getConnectionProfileName(targetProfileId);
    ...
    if (originalProfile === targetProfileName) return fn();

    try {
        console.log(`[TunnelVision] Switching to connection profile: "${targetProfileName}"`);
        await profileCmd.callback({ await: 'true', timeout: '3000' }, targetProfileName);
        return await fn();
    } finally {
 ...
 * @param {number} [options.contentLimit] - Optional max characters of entry content/preview to include
 * @returns {string}
 */
function formatEntryForLLM(entry, detail, options = {}) {
    const { includeUid = true, contentLimit = null } = options;
    const label = entry.comment || entry.key?.[0] || `Entry #${entry.uid}`;
    const fullContent = entry.content || '';
    const resolvedLimit = Number.isFinite(Number(contentLimit)) && Number(contentLimit) > 0
        ? Number(contentLimit)
        : null;
    ...
    if (detail === 'lite') {
        const previewLimit = resolvedLimit ? Math.min(resolvedLimit, 150) : 150;
        const preview = fullContent.substring(0, previewLimit);
        if (preview) line += `\n    Preview: ${preview}`;
    } else if (detail === 'full') {
        const content = resolvedLimit ? fullContent.substring(0, resolvedLimit) : fullContent;
        if (content) line += `\n    Content: ${content}`;
    }
