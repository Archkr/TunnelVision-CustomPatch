...
import { world_names, loadWorldInfo, createWorldInfoEntry, saveWorldInfo } from '../../../world-info.js';
...
import { getActiveTunnelVisionBooks, ALL_TOOL_NAMES, preflightToolRuntimeState } from './tool-registry.js';
...
export async function runDiagnostics() {
    const settingsSnapshot = JSON.stringify(getSettings());
    const results = [];
    ...
    if (JSON.stringify(getSettings()) !== settingsSnapshot) {
...
function checkActivityFeedEvent() {
    ...
    if (!event_types || !event_types.WORLD_INFO_ACTIVATED) {
        results.push(warn('event_types.WORLD_INFO_ACTIVATED not found. Native world-info activations will not populate the Entries tab.'));
    } else {
        results.push(pass('WORLD_INFO_ACTIVATED event available for native entry tracking'));
    }
    if (!event_types || !event_types.TOOL_CALLS_PERFORMED) {
        results.push(warn('event_types.TOOL_CALLS_PERFORMED not found. Activity feed will not show real-time tool calls or TunnelVision retrieval entries.'));
    } else {
        results.push(pass('TOOL_CALLS_PERFORMED event available for tool tracking and TunnelVision retrieval entries'));
    }
    if (!event_types || !event_types.TOOL_CALLS_RENDERED) {
        results.push(warn('event_types.TOOL_CALLS_RENDERED not found. Hidden TunnelVision tool-call chat messages cannot be re-hidden after render.'));
    } else {
        results.push(pass('TOOL_CALLS_RENDERED event available for TunnelVision chat-visibility mode'));
    }
    ...
    results.push(pass('Entries tab semantics: native WI activations come from WORLD_INFO_ACTIVATED, TunnelVision retrievals come from tool results'));
    return results;
}
...
function checkStealthMode() {
    ...
    if (settings.stealthMode === true) {
        return warn('Hide TunnelVision tool-call messages: ON. Pure TunnelVision tool-call system messages are hidden visually, but recursion and the feed remain active.');
    }
    return pass('Hide TunnelVision tool-call messages: off (tool-call messages visible in chat)');
}
